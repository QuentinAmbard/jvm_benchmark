...
class Test:
    ...
    def startGatlingTest(self):
        ...
        print(command)
        process_injector = subprocess.call(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        command_sar = "sarviewer-master/data_collector.sh -n %d -i 1 && cp cp -r sarviewer-master/graphs/* %s && /opt/jdk1.8.0_161/bin/jinfo `pgrep -f cassandra` > " % (testDurationSec + 20, outputFolder+"/"+self.name+"-sar", outputFolder+"/"+self.name+"-sar/jvm.info")
        print(command_sar)
        process_sar = subprocess.call(command_sar, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
